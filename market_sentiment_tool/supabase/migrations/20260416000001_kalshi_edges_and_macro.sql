-- Folded in from shared/migrations/001_add_kalshi_and_macro_tables.sql so
-- these tables are tracked by the Supabase CLI like everything else instead
-- of living in an untracked sibling folder. Idempotent: safe to run even if
-- these were already applied manually via the SQL editor against
-- project wuhpbvgidnrrdndhkehl.
...
DROP POLICY IF EXISTS "kalshi_edges_owner" ON kalshi_edges;
...
DROP POLICY IF EXISTS "macro_signals_owner" ON macro_signals;
...
DROP POLICY IF EXISTS "fpl_optimizations_owner" ON fpl_optimizations;
